...
class EasyMedicalPredictor:
    ...
    def classify_disease_type(self, image_tensor):
        ...
        results = []
        if b_class: results.append({'disease': 'Brain Tumor', 'confidence': b_conf})
        if p_class: results.append({'disease': 'Pneumonia', 'confidence': p_conf})
        
        if not results:
            return None, None
        
        # Return the disease type with highest confidence
    ...
    def predict_and_visualize(self, image_path, disease_type):
        ...
        # First classify the actual disease type in the image
        ...
        # Check if user selection matches actual disease
        if actual_disease.lower().replace(' ', '_') != disease_type:
            print(f"\n⚠️  WARNING: Image appears to be {actual_disease}, but you selected {disease_type.replace('_', ' ').title()}")
            print("The system will analyze with the appropriate model for the actual disease type.")
        
        # Get prediction based on actual disease type (not user selection)
        if actual_disease == 'Brain Tumor':
            b_class, b_conf, b_probs = self.predict_brain_tumor(image_tensor)
            if b_class:
                best = {'disease': 'Brain Tumor', 'prediction': b_class, 'confidence': b_conf, 'model': self.brain_tumor_model, 'details': b_probs, 'classes': self.brain_tumor_classes}
            else:
            ...
            p_class, p_conf, p_probs = self.predict_pneumonia(image_tensor)
            if p_class:
                best = {'disease': 'Pneumonia', 'prediction': p_class, 'confidence': p_conf, 'model': self.pneumonia_model, 'details': p_probs, 'classes': self.pneumonia_classes}
            else:
